...
export function redactSecrets(value) {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [
    key,
    ["password", "api-key", "sid"].includes(key.toLowerCase()) ? "[redacted]" : redactSecrets(item)
  ]));
}
        ...
        res.on("end", () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            reject(new CheckPointApiError(
              parsed.message || parsed.errors?.[0]?.message || `${command} failed with HTTP ${res.statusCode}.`,
